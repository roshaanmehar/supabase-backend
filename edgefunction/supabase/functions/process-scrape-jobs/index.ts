...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: jobs, error: queryError } = await supabase.rpc("get_undone_scrape_jobs");
    if (queryError) {
    ...
    for (const job of jobs){
      try {
        ...
        if (!flaskResult.success) {
          console.error(`Flask API returned failure for job ${job.job_id}:`, flaskResult.error);
          failedJobs.push({
            job_id: job.job_id,
        ...
        const { error: updateJobError } = await supabase.from("scrape_jobs").update({
          status: "ongoing"
        }).eq("id", job.job_id);
        if (updateJobError) {
        ...
        const { error: updatePartsError } = await supabase.from("scrape_job_parts").update({
          status: "ongoing"
        }).eq("job_id", job.job_id);
        if (updatePartsError) {
          console.error(`Failed to update job parts for ${job.job_id}:`, updatePartsError);
          failedJobs.push({
            job_id: job.job_id,
